...
docker run --network=host -v `pwd`/tests/:/tests direktiv/karate java -DtestURL=${DIREKTIV_TEST_URL} -Dlogback.configurationFile=/logging.xml -Dapikey="${DIREKTIV_SECRET_apikey}"  -jar /karate.jar /tests/v1.0/karate.yaml.test.feature ${*:1}